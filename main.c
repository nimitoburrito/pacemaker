...
//    unsigned int uiSwitchValue = 0;
    ...
    while (1) {
    	...
    	if(implementation == 0) {
    		...
    		// print to console
    		//printf("C - AS: %d, VS: %d, AP: %d, VP: %d AEI Value: %d VTO: %d, LRI: %d, AVI: %d\n", AS_input, VS_input, AP_output, VP_output, AEI_timer, VRP_timeout, LRI_timer, AVI_timer);

			printf("LRI: %d, URI: %d, PVARP: %d, VRP: %d, AVI: %d, AEI: %d\n", LRI_timer, URI_timer, PVARP_timer, VRP_timer, AVI_timer, AEI_timer);
    		// LED output based on AP and VP
    		uint32_t gled_output = 0x00;
    		if (VP_output) {
				gled_output |= 0x01;
